...
const DividendCalendar = () => {
  ...
  >
    <div
      className="bg-gradient-to-br from-gray-900 to-gray-950 rounded-3xl p-8 max-w-5xl w-full max-h-[90vh] overflow-y-auto m-4 border border-blue-700/30 shadow-2xl animate-fade-in"
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {selectedDateEvents.events.map((stock, index) => (
          <div
            key={index}
            className="relative group bg-gray-800/60 rounded-2xl p-4 shadow-lg hover:shadow-red-400/20 transition-all border border-gray-700 hover:border-red-400 cursor-pointer"
            onClick={() => handleEventClick(stock)}
          >
            {/* Glowing border animation */}
            <div className="absolute inset-0 pointer-events-none z-0 rounded-2xl group-hover:animate-border-glow border-2 border-transparent group-hover:border-red-500"></div>

            {/* Stock logo */}
            <div className="z-10 relative flex flex-col items-center">
              <div className="w-16 h-16 bg-white rounded-xl overflow-hidden mb-3 shadow-inner flex items-center justify-center transition-transform duration-200 transform group-hover:scale-110">
                <img
              ...
              <p className="text-base font-semibold text-center bg-gradient-to-r from-red-400 to-red-300 bg-clip-text text-transparent">
                {stock.symbol}
              </p>
              <p className="text-sm text-gray-300 text-center mt-1">{stock.companyName}</p>
            </div>
          </div>
        ))}
      </div>
