  ...
  useEffect(() => {
    const fetchDividendData = async () => {
      try {
        ...
        // Fetch quantity data from Supabase
        const { data: quantityData, error: quantityError } = await supabase
          .from('quantity')
          .select('symbol, quantity');

        if (quantityError) {
          console.error('Error fetching quantity data:', quantityError);
        }

        // Create a map to store quantities
        const quantityMap = new Map<string, number>();
        if (quantityData) {
          quantityData.forEach((item: { symbol: string; quantity: number }) => {
            quantityMap.set(item.symbol.toUpperCase(), item.quantity);
          });
        }
      ...
      {selectedDateEvents && (
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-x-3 gap-y-4 px-1">
              {selectedDateEvents.events.map((stock, index) => (
                <div
                  key={index}
                  className="flex flex-col items-center cursor-pointer group relative w-[45px]"
                  onClick={() => handleEventClick(stock)}
                >
                  {/* Stock card with fixed dimensions */}
                  <div className="relative w-[45px] h-[55px] border border-red-500 rounded-sm overflow-hidden">
                  ...
                  {/* Symbol and quantity text */}
                  <div className="w-[45px] bg-[#1e293b] py-1 text-center group-hover:bg-red-900/80 transition-colors border border-red-500 mt-1 rounded-sm overflow-hidden">
                    <p className="text-xs font-bold text-red-400 group-hover:text-red-300 truncate px-1">
                      {stock.symbol}
                    </p>
                    {stock.quantity && (
                      <p className="text-[10px] text-gray-400 mt-0.5 truncate px-1">
                        Qty: {stock.quantity}
                      </p>
                    )}
                  </div>
