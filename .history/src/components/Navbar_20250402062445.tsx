        ...
        <div className="absolute z-50 mt-1 w-full bg-gray-800 border border-gray-700 rounded-lg shadow-lg">
          {filteredStocks
            ...
            .sort((a, b) => {
              ...
              if (aSymbolLower === termLower) return -1;
              if (bSymbolLower === termLower) return 1;

              // Priority 2: Symbol starts with search term
              const aSymbolStarts = aSymbolLower.startsWith(termLower);
              const bSymbolStarts = bSymbolLower.startsWith(termLower);
              if (aSymbolStarts && !bSymbolStarts) return -1;
              if (!aSymbolStarts && bSymbolStarts) return 1;

              // Priority 3: Title starts with search term
              const aTitleStarts = aTitleLower.startsWith(termLower);
              const bTitleStarts = bTitleLower.startsWith(termLower);
              if (aTitleStarts && !bTitleStarts) return -1;
              if (!aTitleStarts && bTitleStarts) return 1;

              // Priority 4: Symbol contains search term
              const aSymbolIndex = aSymbolLower.indexOf(termLower);
              const bSymbolIndex = bSymbolLower.indexOf(termLower);
              if (aSymbolIndex !== -1 && bSymbolIndex === -1) return -1;
              if (aSymbolIndex === -1 && bSymbolIndex !== -1) return 1;
              if (aSymbolIndex !== bSymbolIndex) return aSymbolIndex - bSymbolIndex;

              // Priority 5: Title contains search term
              const aTitleIndex = aTitleLower.indexOf(termLower);
              const bTitleIndex = bTitleLower.indexOf(termLower);
              if (aTitleIndex !== bTitleIndex) return aTitleIndex - bTitleIndex;

              // Default: alphabetical order by symbol
