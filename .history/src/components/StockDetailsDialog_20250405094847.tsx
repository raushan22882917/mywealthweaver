        ...
        fetchDividendData(stock.Symbol).then(data => {
          if (data) {
            ...
            setLatestDividends({
              ...
              annual: data.dividend?.toString() || '',
              quarterly: data.dividend ? (data.dividend / 4).toFixed(2) : '',
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-5 gap-4 mb-6">
            ...
            <Card className={`mt-4 p-4 ${theme === "dark" ? 'bg-gray-800' : 'bg-black'}`}>
              ...
              <div className="text-lg font-bold">
                {latestDividends.dividend ?
                  Number(latestDividends.dividend) > 0 ? 'Dividend Paying' : 'Non-Dividend' :
                  'Unknown'}
              </div>
            </Card>
            <Card className={`mt-4 p-4 ${theme === "dark" ? 'bg-gray-800' : 'bg-black'}`}>
              ...
              <div className="text-lg font-bold">
                {latestDividends.dividend && Number(latestDividends.dividend) > 0 ?
                  `${((Number(latestDividends.dividend) / 100) * 100).toFixed(2)}%` :
                  'N/A'}
              </div>
            </Card>
          </div>
