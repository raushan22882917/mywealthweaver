...
const DividendCountdown: React.FC<{ symbol: string }> = ({ symbol }) => {
  ...
  useEffect(() => {
    const fetchDates = async () => {
      try {
        ...
        setDates({ exDividendDate: null, payoutDate: null });
      }
    };
  ...
  useEffect(() => {
    if (!dates.exDividendDate && !dates.payoutDate) return;

    const calculateTimeLeft = () => {
      const now = new Date(new Date().toLocaleString("en-US", { timeZone: "America/New_York" }));

      let exDividendDifference = 0;
      let payoutDifference = 0;

      if (dates.exDividendDate) {
        const exDividendDate = new Date(dates.exDividendDate);
        exDividendDate.setHours(16, 0, 0, 0);
        exDividendDifference = exDividendDate.getTime() - now.getTime();
      }
      ...
      setTimeLeft({
        exDividendDays: Math.max(0, Math.floor(exDividendDifference / (1000 * 60 * 60 * 24))),
        exDividendHours: Math.max(0, Math.floor((exDividendDifference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))),
        ...
        isExDividendPassed: exDividendDifference < 0,
  ...
  if (!dates.exDividendDate && !dates.payoutDate) {
    return (
            ...
            {!timeLeft.isExDividendPassed ? (
              <>
                ...
                <div className="flex justify-center items-center gap-12">
                  {[
                    { label: "Days", value: timeLeft.exDividendDays, color: "text-white" },
