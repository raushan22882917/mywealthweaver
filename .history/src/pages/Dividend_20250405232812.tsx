...
import { ChevronLeft, ChevronRight, Plus, X, CalendarIcon, Bell } from "lucide-react";
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        const { data, error } = await supabase
          .from('news')
          .select('symbol, content')
          .eq('type', 'dividend');

        if (error) throw error;

        const announcements = (data || []).reduce((acc: {[key: string]: string}, curr) => {
          acc[curr.symbol] = curr.content;
          return acc;
        }, {});

        setDividendAnnouncements(announcements);
        console.log('Loaded dividend announcements from Supabase:', Object.keys(announcements).length);
      } catch (error) {
