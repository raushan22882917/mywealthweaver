...
const Dividend: React.FC = () => {
  ...
  const handleStockClick = useCallback((stock: DividendData, event: React.MouseEvent) => {
    ...
    const prevClick = clickCount[key];

    // Check if this is a double click (within 300ms)
    if (prevClick && now - prevClick.timestamp < 300 && prevClick.count === 1) {
      // Double click - open the stock details dialog
      setClickCount(prev => ({
        ...prev,
        [key]: { count: 0, timestamp: now }
      }));
      ...
      // Single click - show the hover details
      setClickCount(prev => ({
        ...prev,
        [key]: { count: 1, timestamp: now }
      }));
  ...
  }, [autoCloseTimer, clickCount, setDialogOpen, setSelectedStock]);
