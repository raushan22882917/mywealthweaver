...
export default function Dashboard({ session }: DashboardProps) {
  ...
  useEffect(() => {
    const loadCompanyLogos = async () => {
      try {
        const response = await fetch('/logos.csv');  // Updated path to your CSV
  ...
  const fetchUserStocks = async (userId: string) => {
    try {
      ...
      // Match logos with stocks, using database first, then CSV as fallback
      const stocksWithLogos = stocksData?.map(stock => {
        ...
        // If not in database, try to get from CSV
        const csvLogo = companyLogos.find(logo => logo.Symbol.toUpperCase() === upperSymbol);
        return {
          ...stock,
          LogoURL: csvLogo?.LogoURL || '/stock.avif' // Fallback to default image if no logo found
        };
      });
