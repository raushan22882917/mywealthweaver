...
const DividendYield: React.FC = () => {
  ...
  // Get the stock symbol from the URL or search params
  const stockSymbol = symbol || searchParams.get("symbol") || "HEQ";
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const formattedData = data.map((item) => ({
          ...
          yield: parseFloat(item.yield.toFixed(2)),
          dividends: parseFloat(item.dividends.toFixed(2)),
        }));
        ...
        setYieldData([]);
        setChartData([]);
        setError("No yield data found for this symbol.");
      }
    } catch (err: any) {
